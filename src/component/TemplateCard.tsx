    ...
    >
      <div className="flex justify-center w-full flex-row">
        <div className="w-full h-12 flex items-center relative">
          {icon === "" ? (
            <span className="text-3xl font-bold h-full w-12 border border-gray-300 rounded-md flex justify-center items-center">
              {title[0].toUpperCase()}
            </span>
          ) : (
            <img
              src={icon}
              alt={title}
              className="h-full w-12 border border-gray-300 rounded-md"
            />
          )}
          <div className="h-full absolute right-0 flex items-end">
            <span className="text-xl font-bold inline-block align-bottom">
              {title}
            </span>
          </div>
        </div>
